...
import tokyo.mstp015v.timetable_pro.databinding.ActivityMainBinding
import tokyo.mstp015v.timetable_pro.realm.TimeTableDetail
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //toolbarの設定
        binding.toolbar.setTitle(getString(R.string.timetable_app))
    ...
    fun spreadsheetToRealmRestore(credential: GoogleAccountCredential){
        ...
        val at = realm.where<TimeTableDetail>().findAll()
        realm.executeTransaction{
        ...
        MainScope().launch{
            ...
            realm.executeTransaction{
                ...
                it.where<TimeTableDetail>().findAll().deleteAllFromRealm()
            ...
            at_res!!.getValues()?.forEach { row ->
                if( cnt > 0 ) {
                    realm.executeTransaction  {
                        val maxid = it.where<TimeTableDetail>().max("id")
                        val nextid = (maxid?.toLong() ?: 0L) + 1L
                        val rat = it.createObject<TimeTableDetail>( nextid )
    ...
    fun realmToSpreadsheetBackup(credential: GoogleAccountCredential){
        ...
        val at = realm.where<TimeTableDetail>().findAll()
        sheetmap.put("timetabledetail",createValueList().apply{
            at.forEach{
                this.add( mutableListOf(
                    it.id,it.g_name,it.year,
                    it.month,it.date,it.timed,it.sub_name,it.sub_mentor,it.at_code,it.memo
                ))
            }
        })
        ...
        val tc = realm.where<TimeTableColor>().findAll()
        sheetmap.put("timetable",createValueList().apply{
            tc.forEach{
                this.add( mutableListOf(
                    it.id,it.g_name,it.sub_name,it.color
                ))
            }
        })
        sheetmap.forEach{
    ...
    fun newAttendanceSheet( credential : GoogleAccountCredential,drive:Drive,folder_id:String ) : String{
        ...
        val sheetNames = arrayListOf("group","timetable","timetabledetail","timetablecolor")
        ...
        sheetmap.put("timetabledetail",createValueList().apply{
            this.add( mutableListOf("id","g_name","year","month",
                "daye","timed","sub_name","sub_mentor","at_code","memo"))
        })
        sheetmap.put("timetable",createValueList().apply{
            this.add(mutableListOf("id","g_name","day","timed","sub_name","sub_mentor"))
        })
        sheetmap.put("timetablecolor",createValueList().apply{
            this.add(mutableListOf("id","g_name","sub_name","color"))
        })
        sheetmap.forEach{
