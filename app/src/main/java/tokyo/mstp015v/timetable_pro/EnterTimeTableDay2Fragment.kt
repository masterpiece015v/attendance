...
import tokyo.mstp015v.timetable_pro.databinding.FragmentEnterTimeTableDay2Binding
...
class EnterTimeTableDay2Fragment(var g_name:String,
    ...
    override fun onResume() {
        ...
        //チェックリストの作成
        list.clear()
        val checkret = realm.where<TimeTableDetail>()
        ...
        data class TimeTableMini(var timed:Int,var sub_name:String,var sub_mentor:String,var color:Int)
        val itemlist = mutableListOf<TimeTableMini>()

        for( i in 1..max_timed){
            //timetabledetailにあればそれを使う
            val ret1 = realm.where<TimeTableDetail>()
                .equalTo("g_name",g_name)
                .equalTo("year",year)
                .equalTo("month",month)
                .equalTo("date",date)
            ...
            if( ret1 != null ){
                val item = TimeTableMini(i,ret1.sub_name,ret1.sub_mentor,ret1.color)
                itemlist.add( item )
            }else{
                ...
                val ret2 = realm.where<TimeTable>()
                    ...
                    if( ret2 != null ){
                        val item = TimeTableMini( i,ret2.sub_name,ret2.sub_mentor,ret2.color)
                        itemlist.add( item )
                    }else{
                        val item = TimeTableMini( i ,"","",Color.parseColor("#FFFFFF"))
                        itemlist.add( item )
                    }
            }
        }
        ...
        itemlist.forEach{db->
            ...
            linearTrush.setBackgroundColor( db.color )
            //クリックイベント
            ...
            linearTrush.setOnLongClickListener{
                ...
                buttonTrush.setOnClickListener {
                    realm.executeTransaction{ deleteDb->
                        val ret = deleteDb.where<TimeTableDetail>()
                            .equalTo("g_name",g_name)
                            .equalTo("year",year)
                            .equalTo("month",month)
                            .equalTo("date",date)
